...
class WhatsAppInstance {
  ...
  // axiosInstance = axios.insert({
  //     baseURL: config.webhookUrl,
  // })

  constructor(key, userId, allowWebhook = false) {
    console.log("start wa key - " + key + ", userId - " + userId + ", allowWebhook - " + allowWebhook);
  ...
  setHandler() {
    ...
    sock?.ev.on("connection.update", async (update) => {
      ...
      if (qr) {
        ...
        // let { instanceId } = await Instance.findOne({ _id: this.key})

        await Instance.updateOne(
          { _id: this.key },
          {$set: {status: "QR Generated" ,message: "Scan Qr for login" , qr:qrImg }}
        );
        ...
        await Instance.updateOne(
          { _id: this.key },
          {$set: { status: status, message: message, number: "" , qr:""}}
        );
        ...
        await Instance.updateOne(
          { _id: this.key },
          {$set: { status: "Ready", message: "Connection success" , qr:'' }}
        );
    ...
    sock?.ev.on("messages.upsert", async (m) => {
      ...
      if (!msg.key.fromMe) {
        ...
        const m = await ReceivedMessage.insert({
          instanceId: this.key,
          userId: this.userId,
          message: msgBody,
          number: number,
